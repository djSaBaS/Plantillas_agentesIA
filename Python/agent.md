...
No se permite mezclar lógicas backend en otros lenguajes dentro del mismo repositorio, salvo que exista una arquitectura formalmente documentada que lo justifique (por ejemplo, un componente externo consumido por API).
...
- El comentario debe explicar qué hace la línea o por qué existe (priorizar el “por qué”).
...
- Validar, normalizar y sanear entradas provenientes de:
  - Argumentos de CLI
  - Ficheros
  - Red/APIs
  - UI/inputs del usuario
- Prohibido exponer secretos en logs, trazas o errores.
- Manejo controlado de excepciones:
  - Mensajes claros al usuario
  - Detalle técnico solo donde corresponda (y sin secretos)
- Principio de mínimo privilegio (permisos, tokens, rutas).

---

### 5.1 Ejecución de comandos y sistema

- Evitar `eval`/`exec`. Si fuera imprescindible, debe justificarse y aislarse.
...
- Preferir listas de argumentos (`["cmd", "arg1"]`) y allowlist de opciones.
...
- Debe existir plantilla de configuración cuando aplique (por ejemplo `.env.example` o `config.example.*`).
...
El proyecto debe mantenerse sin alertas críticas en el entorno habitual (VS Code + herramientas de lint/format/test del repo).
...
- Debe existir un listado de dependencias (`requirements.txt` o equivalente).
- Debe documentarse en `README.md`:
...
- Tests de regresión: cada bug corregido añade un test que lo reproduzca.
...
- No se suben secretos (por ejemplo `.env`).
